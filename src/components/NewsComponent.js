...
import NewsHeadLinesComp from "./NewsHeadLinesComp";
import { BsSave } from "react-icons/bs";
import { useState } from "react";
import PaginationComp from "./paginationComp";

const ShowNewsComponent = () => {
  const [favouriteArticles, setFavouriteArticles] = useState([]); //there may be array of articles that's why we are storing them in am state
  ...
  return NewsData !== null ? (
    <>
      <div className="w-full">
                ...
                {index === 0 ? (
                  ...
                  <div className="mt-4 border rounded-md border-gray-200 p-3 h-auto flex justify-around flex-wrap ">
                    ...
                    <div className="w-1/2 float-left mt-6">
                      ...
                      {/* here we are storing the article in local storage */}
                      <BsSave
                        className="float-left mt-5 size-5 hover:scale-125 cursor-pointer"
                        onClick={() => {
                          favouriteArticles.length === 0 &&
                            setFavouriteArticles(n);
                          let favArticles = [...favouriteArticles];
                          favArticles.push(n);
                          setFavouriteArticles(favArticles);
                          localStorage.setItem(
                            "articles",
                            JSON.stringify(favouriteArticles)
                          );
                        }}
                      />
                    </div>
        ...
        {/* when the width is above 1024 px it show on the right of top news */}
        <div className="w-[40%] float-right max-lg:hidden">
          <NewsHeadLinesComp />
        </div>
      </div>
      {/* when the width is below 1024 or below 640px it will come in the bottom  */}
      <div className="w-[60%] float-left ml-56 lg:hidden max-sm:w-fit max-sm:ml-2 max-lg:w-fit  max-lg:ml-2">
        <NewsHeadLinesComp />
      </div>
    </>
  ) : (
    // when there is no data in the news store then it will show the loading indicator
